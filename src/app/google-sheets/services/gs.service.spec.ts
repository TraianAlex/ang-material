...
describe('GoogleSheetsDbService', () => {
  ...
  it('should convert rows to entries', () => {
    ...
    expect(service['rowsToObjects'](rows)).toEqual(entries);
  });
  ...
  it('should cleant column names', () => {
    ...
    expect(service['cleanColumnName'](' Test 2')).toBe('Test 2');
    expect(service['cleanColumnName']('  As_dö1+l-?r#^2  t=e*s$t  ')).toBe('As_dö1+l-?r#^2  t=e*s$t');
    expect(service['cleanColumnName']('TÍTULO')).toBe('TÍTULO');
    expect(service['cleanColumnName']('ESPECIFICAÇÃO da TECNOLOGIA')).toBe('ESPECIFICAÇÃO da TECNOLOGIA');
  });
  ...
  it('should read Google sheet and return all entries', (done: DoneFn) => {
    service.get('Characters', attributesMapping).subscribe((data: any) => {
  ...
  it('should read Google sheet and return active entries', (done: DoneFn) => {
    service
      .getActive('Characters2', attributesMapping, 'Active')
      .subscribe((data: any) => {
  ...
  it('should read Google sheet and return active entries with custom activeValues list', (done: DoneFn) => {
    service
      .getActive('Characters Test', attributesMapping, 'Active', ['no'])
      .subscribe((data: any) => {
  ...
  it('should read Google sheet and return active entries with custom activeValue', (done: DoneFn) => {
    service
      .getActive('Characters äöü', attributesMapping, 'Active', 'no')
      .subscribe((data: any) => {
        expect(data.length).toBe(2);
